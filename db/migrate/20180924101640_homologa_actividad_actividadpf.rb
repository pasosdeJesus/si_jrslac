class HomologaActividadActividadpf < ActiveRecord::Migration[5.2]
  def up
    execute <<-SQL
      ...
      INSERT INTO cor1440_gen_actividad_actividadpf (actividad_id, actividadpf_id) (SELECT actividad_id, actividadtipo_id+100 FROM cor1440_gen_actividad_actividadtipo);
    SQL
  end
  def down
    execute <<-SQL
      DELETE FROM cor1440_gen_actividad_actividadpf WHERE (actividad_id, actividadpf_id) IN (SELECT actividad_id, actividadtipo_id+100 FROM cor1440_gen_actividad_actividadtipo);
